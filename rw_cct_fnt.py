...
def walk(i, obs):
    ...
    nn = [] #Number of new nodes seen on the walk
    visited = {}
    num_visited=0
    for node in range(total_num_vertices):
        visited[node]=False
    ...
    while 0 in obs:
        ...
        nn.append(num_visited)
        j = random.choice(neigh_dict[i])
        i = j
    return [nc, nn] #Returns the number of classes seen on the walk
...
classes = prop[:, 1:]
#vertex_list = prop[:, 0]
total_num_vertices = np.shape(prop)[0]
...
for node in range(start, end):
    ...
    for n in range(num):
        [series, num_nodes]=walk(node, obs)
        num_nodes_visited.append(num_nodes[-1])
    ...
    print("Node:", node) 
    #print("Threshold values:", *th_values)
    avg_array.append(th_values)
    avg_num_visited=sum(num_nodes_visited)/len(num_nodes_visited)
    avg_frac_visited=avg_num_visited/total_num_vertices
    frac_array.append(avg_frac_visited)
    #print("Fraction of nodes that have been visited:", avg_frac_visited) 
    #print(total_num_vertices)
avg_array=np.asarray(avg_array)
avg_frac_overall = sum(frac_array)/len(frac_array)
print("Average fraction of nodes visited:", avg_frac_overall)
